		...
		if (url.length() == 0){
			...
			mChecker.checkAccess(new LicenseCheckerCallback(){
				...
				public void allow(int reason){
					...
					if (count > 0){
						String zip_dir = Environment.getExternalStorageDirectory() + "/Android/obb/" + getApplicationContext().getPackageName();
						...
						downloader = new DataDownloader(zip_dir, zip_filename, gCurrentDirectoryPath, version_filename, url, file_size, handler);
					}
				}
			...
			downloader = new DataDownloader(zip_dir, zip_filename, gCurrentDirectoryPath, version_filename, url, -1, handler);
		}
	}
